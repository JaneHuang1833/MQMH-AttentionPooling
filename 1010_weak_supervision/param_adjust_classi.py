...
# 男性
idx_male = df["Gender"] == 1
sc_male = plt.scatter(
    Y_best[idx_male, 0], Y_best[idx_male, 1],
    label='male',
    marker='o',
    c=df.loc[idx_male, 'arva'].astype(int).to_numpy(),
    cmap=cmap,
    norm=norm,
    s=40,
    edgecolors='none'
...
# 女性
idx_female = df["Gender"] == 0
plt.scatter(
    Y_best[idx_female, 0], Y_best[idx_female, 1],
    label='female',
    marker='*',
    c=df.loc[idx_female, 'arva'].astype(int).to_numpy(),
    cmap=cmap,
    norm=norm,
    s=40,
    edgecolors='none'
...
# 用 sc_male（第一个 scatter 返回的对象）创建 colorbar
plt.colorbar(sc_male, label='arva')

plt.legend(title="Gender")
plt.title(f"Embedding Visualization by Gender and Arousal*Valence")
plt.xlabel("Dim1")
plt.ylabel("Dim2")
plt.tight_layout()
plt.show()
