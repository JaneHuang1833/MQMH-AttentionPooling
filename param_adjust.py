...
EXCEL_PATH = os.path.join(DATA_DIR, "101400072.xlsx")
...
for params in ParameterGrid(param_grid):
    ...
    nmi = compute_nmi(Y, df["Arousal"].values, n_clusters=5)
    print(f"NMI：{nmi}")
    if nmi > best_nmi:
        best_nmi = nmi
        best_params = params
...
plt.title("Embedding Visualization by Gender and Arousal_UMAP")
plt.xlabel("Dim1")
plt.ylabel("Dim2")
plt.tight_layout()
plt.show()
