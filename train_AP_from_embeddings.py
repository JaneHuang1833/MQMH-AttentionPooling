...
def train(args):
    ...
    with open(args.train_csv) as f:
        r=_csv.DictReader(f); first=next(iter(r))
    ...
    for ep in range(1, args.epochs+1):
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save({"model":model.state_dict(),"in_dim":in_dim,"labels":id2label,"args":vars(args)}, best_path)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--labels", required=True, help="逗号分隔，如: angry, disgusted, fearful, happy, other, sad, surprised")
